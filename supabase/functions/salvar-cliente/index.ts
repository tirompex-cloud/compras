...
Deno.serve(async (req: Request) => {
  ...
  if (dados.id) {
    const { id, ...rest } = dados;
    result = await supabase.from("clientes").update(rest).eq("id", id).select().single();
  } else {
    const { id: _ignored, ...rest } = dados;
    result = await supabase.from("clientes").insert(rest).select().single();
  }
  ...
  return json({ ok: true, cliente: result.data });
});
